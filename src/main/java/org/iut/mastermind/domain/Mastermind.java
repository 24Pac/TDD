...
public class Mastermind {
    ...
    public boolean nouvellePartie(Joueur joueur) {
        Optional<Partie> partieEnregistree = partieRepository.getPartieEnregistree(joueur);
        if (isJeuEnCours(partieEnregistree)) {
            return false;
        }
        String mot = serviceTirageMot.tirageMotAleatoire();
        Partie partie = Partie.nouvelle(joueur, mot);
    ...
    public ResultatPartie evaluation(Joueur joueur, String motPropose) {
        ...
        ResultatPartie resultat = calculeResultat(partie, motPropose);
